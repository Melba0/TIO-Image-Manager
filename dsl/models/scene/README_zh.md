...
转换方式：把官方 `googlenet_places365.caffemodel` + `deploy_googlenet_places365.prototxt`
转换为 ONNX。ONNX 图在 `loss3/classifier` 处截断（365 维 logits，softmax 前）。

## 备选：ResNet18 / ResNet50（PyTorch 检查点）

如果改用 ResNet 系列，用 `torch.onnx` 从官方 `.pth.tar` 导出（注意：导出文件名需为引擎期望的
`places365_googlenet.onnx`，或同时修改 `main.cpp` 中的模型路径）：

```powershell
python -c "import torch,torch.onnx; ..."   # 把 resnet18_places365.pth.tar 导出为 places365_googlenet.onnx
```
