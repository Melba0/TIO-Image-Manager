...
[**中文**](usage_tutorial_zh.md) ・ [**English**](usage_tutorial.md)

本教程介绍解释器各功能模块的实际用法：首次运行、增量缓存、模型注册表、继承查询、
标签预筛选、资产管理、相簿、聚类，以及桌面 GUI 的配合使用。
...
1. 把 `.pt` 转成 `.onnx`（见 [base_model_pack_format.md](base_model_pack_format_zh.md)），
   放入 `models/base/<名字>/model.onnx`。
2. 同目录创建 `meta.json`（`name` 与目录名一致）与 `classes.json`
   （见 [base_model_pack_format.md](base_model_pack_format_zh.md) 的字段规则）。
...
**模型准备**（一次性）：`models/scene/` 下已提供 `places365_googlenet.onnx` +
`categories_places365.txt`。如需从官方 Caffe 权重或 PyTorch 检查点重新导出 ONNX，
请按 `models/scene/README.md` 的约定使用 `torch.onnx` 导出。
...
相簿是 GUI 管理的逻辑分组，存储在 `cache_index.json` 的 `collections` 字段中，**绝不移动磁盘文件**。

- **创建 / 管理**：GUI 左侧边栏右键 📁 相簿分支 → 新建相簿，然后把缩略图拖入（或右键缩略图
  → 添加到相簿）。
- **DSL 查询**：`collection("名称")` 返回相簿图片集，可与筛选、集合运算自由组合（见 DSL 参考）：

```dsl
collection("我的旅行")                              # 相簿中的全部图片
collection("我的旅行") : (any(class == "cat"))      # 在相簿中搜猫
collection("猫图精选") | collection("狗图精选")      # 两个相簿的并集
```

## 5.6 聚类 / 人物分组（V2）

激活聚类扩展包（如 `face_recognition_v1`）后，缓存构建会为每个匹配对象提取 embedding 并对全库
做 DBSCAN 聚类。GUI 左侧边栏会显示分组分支（如 👤 人物）：

- 每个聚类为一个可点击条目（映射名或原始 id + 照片数）；单击在网格中显示该组照片；
- 双击 / 右键 → 重命名，给聚类起一个易读的名字（持久化到 `config/cluster_name_mappings.json`）；
- DSL 中可用聚类宏查询：
...
└── model.onnx    # 分类器 [1,nc] 或检测器 [1,4+nc,anchors] 或嵌入 [1,D]
```
...
新的全局 `obj_id`，并映射回原图坐标。扩展包格式详见 [extension_pack_format.md](extension_pack_format_zh.md)。
